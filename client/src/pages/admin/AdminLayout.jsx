...
import { FiGrid, FiShoppingBag, FiPackage, FiUsers, FiTag, FiPercent, FiTrendingUp, FiLogOut, FiMenu, FiX } from 'react-icons/fi';
    ...
    <div className="admin-layout">
      ...
      <aside className={`admin-sidebar${sidebarOpen ? ' open' : ''}`}>
        ...
        <nav className="admin-nav">
          {navItems.map(item => (
            <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `admin-nav-item${isActive ? ' active' : ''}`} onClick={() => setSidebarOpen(false)}>
      ...
      <div className="admin-main">
        <header className="admin-topbar">
          ...
          <NavLink to="/" className="admin-home-link">← Siteye Dön</NavLink>
        </header>
